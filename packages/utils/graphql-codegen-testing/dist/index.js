...
const nock = _interopDefault(require('nock'));
...
function validateTs(pluginOutput, options = {
    ...
    lib: [
        ...
        path.join(path.dirname(require.resolve('typescript')), 'lib.esnext.d.ts'),
    ],
...
expect.extend({
   ...
   ${expected}`,
            ...
            const diffString = jestDiff.diff(commonTags.stripIndent `${expected}`, commonTags.stripIndent `${received}`, {
                expand: this.expand,
            });
