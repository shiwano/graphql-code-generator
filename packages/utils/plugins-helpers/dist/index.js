...
function resolveExternalModuleAndFn(pointer) {
    ...
    if (moduleName === 'change-case-all') {
        ...
        const cwdRequire = module$1.createRequire(process.cwd());
...
function isFederationObjectType(node, schema) {
    ...
    const rootTypeNames = visitorPluginCommon.getRootTypeNames(schema);
...
function getCachedDocumentNodeFromSchema(schema) {
    ...
    if (!documentNode) {
        documentNode = utils.getDocumentNodeFromSchema(schema);
