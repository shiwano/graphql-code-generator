...
class JavaResolversVisitor extends visitorPluginCommon.BaseVisitor {
    ...
    EnumValueDefinition(node) {
        return (enumName) => {
            return visitorPluginCommon.indent(`${this.getEnumValue(enumName, node.name.value)}`);
        };
    }
    ...
    buildInputTransfomer(name, inputValueArray) {
        ...
        const classMembers = inputValueArray
            ...
            if (arg.name.value === 'interface' || arg.name.value === 'new') {
                ...
                return visitorPluginCommon.indent(`private ${typeToUse.typeName} _${this.config.classMembersPrefix}${arg.name.value};`);
            }
            else {
                return visitorPluginCommon.indent(`private ${typeToUse.typeName} ${this.config.classMembersPrefix}${arg.name.value};`);
            }
        })
            .join('\n');
		...
		this.${arg.name.value} = (${this.config.listType}<${typeToUse.baseType}>) args.get("${arg.name.value}");
                ...
                return visitorPluginCommon.indent(`this.${this.config.classMembersPrefix}${arg.name.value} = (${typeToUse.typeName}) args.get("${arg.name.value}");`, 3);
            }
  ...
  this.${this.config.classMembersPrefix}${arg.name.value} = ${typeToUse.typeName}.valueOf((String) args.get("${arg.name.value}"));
        ...
        const getters = inputValueArray
            ...
            if (arg.name.value === 'interface' || arg.name.value === 'new') {
                // forcing prefix of _ since interface is a keyword in JAVA
                return visitorPluginCommon.indent(`public ${typeToUse.typeName} get${this.convertName(arg.name.value)}() { return this._${this.config.classMembersPrefix}${arg.name.value}; }`);
            }
            else {
                return visitorPluginCommon.indent(`public ${typeToUse.typeName} get${this.convertName(arg.name.value)}() { return this.${this.config.classMembersPrefix}${arg.name.value}; }`);
            }
        })
            .join('\n');
        const setters = inputValueArray
            ...
            const typeToUse = this.resolveInputFieldType(arg.type);
            if (arg.name.value === 'interface' || arg.name.value === 'new') {
                return visitorPluginCommon.indent(`public void set${this.convertName(arg.name.value)}(${typeToUse.typeName} _${arg.name.value}) { this._${arg.name.value} = _${arg.name.value}; }`);
            }
            else {
                return visitorPluginCommon.indent(`public void set${this.convertName(arg.name.value)}(${typeToUse.typeName} ${arg.name.value}) { this.${arg.name.value} = ${arg.name.value}; }`);
            }
        })
            .join('\n');
