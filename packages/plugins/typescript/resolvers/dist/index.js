...
class TypeScriptResolversVisitor extends visitorPluginCommon.BaseResolversVisitor {
    constructor(pluginConfig, schema) {
        super(pluginConfig, {
            ...
            allowParentTypeOverride: visitorPluginCommon.getConfigValue(pluginConfig.allowParentTypeOverride, false),
            optionalInfoArgument: visitorPluginCommon.getConfigValue(pluginConfig.optionalInfoArgument, false),
        }, schema);
        autoBind(this);
        this.setVariablesTransformer(new typescript.TypeScriptOperationVariablesToObject(this.scalars, this.convertName, this.config.avoidOptionals, this.config.immutableTypes, this.config.namespacedImportName, [], this.config.enumPrefix, this.config.enumValues));
...
const plugin = (schema, documents, config) => {
    ...
    const astNode = pluginHelpers.getCachedDocumentNodeFromSchema(transformedSchema);
    // runs visitor
