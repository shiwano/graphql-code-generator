    ...
    describe('Issues', () => {
      ...
      `);
        ...
        it('#3066 - should respect higher level of fragments usage, and ignore fragments per input', async () => {
            const doTest = async (operationsStr, expected) => {
        ...
        `);
              ...
              `),
                    ...
                    pluginMap: { 'typescript-react-apollo': {} },
                });
        ...
        it('#2365 - Should not add Fragment suffix to import identifier when dedupeOperationSuffix: true', async () => {
            ...
            expect(result.map(o => o.plugins)[0]).toEqual(expect.arrayContaining([
                {
                    add: {
            ...
            expect(getFragmentImportsFromResult(result)).toContain(`import { UserFieldsFragmentDoc, UserFieldsFragment } from './user-fragment.generated';`);
        });
        it('#2365 - Should add Fragment suffix to import identifier when dedupeOperationSuffix not set', async () => {
            ...
            expect(getFragmentImportsFromResult(result)).toContain(`import { UserFieldsFragmentFragmentDoc, UserFieldsFragmentFragment } from './user-fragment.generated';`);
        });
...
`);
    ...
    it('Should prepend the "add" plugin with the correct import, when only using fragment spread', async () => {
        ...
        expect(result.map(o => o.plugins)[1]).toEqual(expect.arrayContaining([
            {
                add: {
                    content: `import * as Types from '../types';\n`,
                },
    ...
    it('should fail when multiple fragments with the same name but different definition are found', () => {
        expect(() => preset.buildGeneratesSection({
            baseOutputDir: './src/',
            config: {},
            presetConfig: {
                cwd: '/some/deep/path',
    ...
    it('Should NOT prepend the "add" plugin with Types import when selection set does not include direct fields', async () => {
        ...
        expect(result.map(o => o.plugins)[0]).not.toEqual(expect.arrayContaining([{ add: { content: `import * as Types from '../types';\n` } }]));
    });
    ...
    it('Should allow external fragments to be imported from packages', async () => {
        ...
        expect(getFragmentImportsFromResult(result)).toContain(`import { UserFieldsFragmentDoc, UserFieldsFragment } from '@fragments';`);
    });
    ...
    it('Should add import to external fragment when its in use (nested fragment)', async () => {
        ...
        expect(getFragmentImportsFromResult(result)).toContain(`import { UserFieldsFragmentDoc, UserFieldsFragment } from './nested/down/here/user-fragment.generated';`);
    });
    ...
    `);
        ...
        const result = await preset.buildGeneratesSection({
            ...
            pluginMap: { typescript: {}, 'typescript-operations': {}, 'typed-document-node': {} },
        });
        expect(getFragmentImportsFromResult(result)).toContain(`import { MyGroupFragmentFragmentDoc, MyGroupFragmentFragment } from './fragments/MyGroupFragment';`);
        expect(getFragmentImportsFromResult(result)).toContain(`import { AnotherGroupFragmentFragmentDoc, AnotherGroupFragmentFragment } from './fragments/AnotherGroupFragment';`);
    });
    ...
    `);
        ...
        expect(getFragmentImportsFromResult(result, 1)).toContain(`import { AuthorFragmentDoc, AuthorFragment, AddressFragmentDoc, AddressFragment } from './author';`);
    });
});
const getFragmentImportsFromResult = (result, index = 0) => result[index].config.fragmentImports
    .map(importStatement => generateFragmentImportStatement(importStatement, 'both'))
    .join('\n');
//# sourceMappingURL=near-operation-file.spec.js.map