export function generateDocs(schema, types) {
    const result = {};
    for (const p of types) {
        const subSchema = schema.definitions[p.identifier];
...
function generateContentForSchema(schema) {
...
${prop.description ? `${prop.description}\n` : ''}${prop.exampleMarkdown
            ? `\n#### Usage Examples\n\n${prop.exampleMarkdown.replace(/## /g, '##### ')}`
            : ''}`;
    })
        .join('\n\n');
}
function printType(def) {
    if (def.type) {
        if (def.enum) {
            return `${def.type} (values: ${def.enum.join(', ')})`;
        }
        if (def.type === 'array') {
            return `${printType(def.items)}[]`;
        }
        return def.type;
    }
    else if (def.$ref) {
        ...
        return def.anyOf.map(t => printType(t)).join(' | ');
    }
    else if (def.oneOf) {
        return def.oneOf.map(t => printType(t)).join(' | ');
    }
    else if (def.allOf) {
        return def.allOf.map(t => printType(t)).join(' & ');
    }
    else {
        return '';
    }
