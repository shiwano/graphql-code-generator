...
async function generate() {
    const program = TJS.getProgramFromFiles([ROOT_FILE, ...[...pluginsConfigurations, ...presetsConfigurations].map(f => f.file)], {
    ...
    const schema = generator.getSchemaForSymbols([ROOT_IDENTIFIER, ...pluginsConfigurations.map(f => f.identifier), ...presetsConfigurations.map(f => f.identifier)], true);
        ...
        anyOf: [
            ...
            },
            {
                ...
                oneOf: pluginsConfigurations.reduce((prev, p) => {
                    const description = `${schema.definitions[p.identifier].description || ''}\n\nFor more details and documentation: https://graphql-code-generator.com/docs/plugins/${p.name}\n\n=> Make sure to include "@graphql-codegen/${p.name}" in your package.json file and install your dependencies.\n\n`;
                    return [
                        ...prev,
                        {
                            const: p.name,
    ...
    const outputRecord = schema.definitions['Types.ConfiguredOutput'];
    ...
    await Promise.all(Object.keys(docsMarkdown).map(identifier => writeFile(join(docsOutDir, `./${identifier}.md`), docsMarkdown[identifier])));
