...
describe('init', () => {
    ...
    it('should use angular related plugins when @angular/core is found', async () => {
        ...
        useInputs({
            ...
            onPlugins: [ENTER],
            onOutput: [ENTER],
            onIntrospection: ['n', ENTER],
            onConfig: [ENTER],
            onScript: ['graphql', ENTER], // use custom npm script
        });
        await init();
        expect(writeFileSpy).toHaveBeenCalledTimes(2);
        const pkg = JSON.parse(writeFileSpy.mock.calls[1][1]);
        const config = load(writeFileSpy.mock.calls[0][1]);
        // should use default output path
        expect(config.generates['src/generated/graphql.ts']).toBeDefined();
        const output = config.generates['src/generated/graphql.ts'];
    ...
    it('should use react related plugins when react is found', async () => {
        ...
        const output = config.generates['src/generated/graphql.tsx'];
        expect(output.plugins).toContainEqual('typescript');
        expect(output.plugins).toContainEqual('typescript-operations');
    ...
    it('should have few default values', async () => {
        ...
        await init();
        const configFile = writeFileSpy.mock.calls[0][0];
        const config = load(writeFileSpy.mock.calls[0][1]);
        const pkg = JSON.parse(writeFileSpy.mock.calls[1][1]);
    ...
    it('should have few default values', async () => {
        ...
        useInputs({
            onTarget: [ENTER],
            onSchema: [options.schema, ENTER],
            onDocuments: [options.documents, ENTER],
            onPlugins: [ENTER],
            onOutput: [options.output, ENTER],
            onIntrospection: ['n', ENTER],
            onConfig: [options.config, ENTER],
    ...
    describe('plugin choices', () => {
        function getAvailable(tags) {
            return getPluginChoices({
                targets: tags,
            }).map((c) => c.value.value);
        }
        function getSelected(tags) {
            return getPluginChoices({
                targets: tags,
            })
                .filter((c) => c.checked)
        ...
        function getPlugins(targets) {
            const tags = getApplicationTypeChoices({
