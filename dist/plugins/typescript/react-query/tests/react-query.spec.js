...
describe('React-Query', () => {
  ...
  `);
    ...
    it('should allow to override TError type', async () => {
        ...
        const out = (await plugin(schema, docs, config));
    ...
    describe('fetcher: custom-mapper', () => {
        it('Should generate query correctly with external mapper', async () => {
            ...
            const out = (await plugin(schema, docs, config));
            expect(out.prepend).toContain(`import { useQuery, UseQueryOptions, useMutation, UseMutationOptions } from 'react-query';`);
      ...
      useMutation<TTestMutation, TError, TTestMutationVariables, TContext>(
        ...
        it("Should generate fetcher field when exposeFetcher is true and the fetcher isn't a react hook", async () => {
            ...
            const out = (await plugin(schema, docs, config));
            expect(out.content).toBeSimilarStringTo(`useTestQuery.fetcher = (variables?: TestQueryVariables) => customFetcher<TestQuery, TestQueryVariables>(TestDocument, variables);`);
        });
      ...
      `);
            ...
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], {}, config)).content).toContain('fetcher<NotificationsQueryQuery, NotificationsQueryQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: false }, config)).content).toContain('fetcher<NotificationsQueryQuery, NotificationsQueryQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: false }, config)).content).toContain('export const useNotificationsQueryQuery = ');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: true }, config)).content).toContain('fetcher<NotificationsQuery, NotificationsQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: true }, config)).content).toContain('export const useNotificationsQuery =');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: true }, config)).content).toContain('fetcher<NotificationsQuery, NotificationsQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: true }, config)).content).toContain('export const useNotificationsQuery =');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: false }, config)).content).toContain('fetcher<NotificationsQuery, NotificationsQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: false }, config)).content).toContain('export const useNotificationsQuery =');
        });
    });
    describe('fetcher: graphql-request', () => {
      ...
      useMutation<TTestMutation, TError, TTestMutationVariables, TContext>(
        ...
        it('Should generate fetcher field when exposeFetcher is true', async () => {
            ...
            const out = (await plugin(schema, docs, config));
            expect(out.content).toBeSimilarStringTo(`useTestQuery.fetcher = (client: GraphQLClient, variables?: TestQueryVariables) => fetcher<TestQuery, TestQueryVariables>(client, TestDocument, variables);`);
        });
        it(`tests for dedupeOperationSuffix`, async () => {
            const ast = parse(/* GraphQL */ `
        query notificationsQuery {
    ...
    describe('fetcher: hardcoded-fetch', () => {
      ...
      useMutation<TTestMutation, TError, TTestMutationVariables, TContext>(
        ...
        it('Should generate fetcher field when exposeFetcher is true', async () => {
            ...
            const out = (await plugin(schema, docs, config));
            expect(out.content).toBeSimilarStringTo(`useTestQuery.fetcher = (variables?: TestQueryVariables) => fetcher<TestQuery, TestQueryVariables>(TestDocument, variables);`);
        });
        it(`tests for dedupeOperationSuffix`, async () => {
            const ast = parse(/* GraphQL */ `
        query notificationsQuery {
    ...
    describe('fetcher: fetch', () => {
      ...
      useQuery<TTestQuery, TError, TData>(
        ...
        it('Should generate fetcher field when exposeFetcher is true', async () => {
            ...
            const out = (await plugin(schema, docs, config));
            expect(out.content).toBeSimilarStringTo(`useTestQuery.fetcher = (variables?: TestQueryVariables) => fetcher<TestQuery, TestQueryVariables>(dataSource.endpoint, dataSource.fetchParams || {}, TestDocument, variables);`);
        });
        it(`tests for dedupeOperationSuffix`, async () => {
            const ast = parse(/* GraphQL */ `
        query notificationsQuery {
    ...
    describe('exposeQueryKeys: true', () => {
        it('Should generate getKey for each query', async () => {
            ...
            const out = (await plugin(schema, docs, config));
            expect(out.content).toBeSimilarStringTo(`useTestQuery.getKey = (variables?: TestQueryVariables) => ['test', variables];`);
        });
    });
    it('Should not generate fetcher if there are no operations', async () => {
        const out = (await plugin(schema, notOperationDocs, {}));
        expect(out.prepend).not.toBeSimilarStringTo(`function fetcher<TData, TVariables>(`);
        const config = {
            fetcher: 'graphql-request',
        };
        const outGraphqlRequest = (await plugin(schema, notOperationDocs, config));
        expect(outGraphqlRequest.prepend).not.toContain(`import { GraphQLClient } from 'graphql-request';`);
    });
});
//# sourceMappingURL=react-query.spec.js.map