...
describe('Vue Apollo', () => {
  ...
  `);
    ...
    describe('Imports', () => {
      ...
      `);
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], {}, { outputFile: '' })).content).toContain('VueApolloComposable.UseQueryReturn<NotificationsQueryQuery, NotificationsQueryQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: false }, { outputFile: '' })).content).toContain('VueApolloComposable.UseQueryReturn<NotificationsQueryQuery, NotificationsQueryQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: true }, { outputFile: '' })).content).toContain('VueApolloComposable.UseQueryReturn<NotificationsQuery, NotificationsQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: true }, { outputFile: '' })).content).toContain('VueApolloComposable.UseQueryReturn<NotificationsQuery, NotificationsQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: false }, { outputFile: '' })).content).toContain('VueApolloComposable.UseQueryReturn<NotificationsQuery, NotificationsQueryVariables>');
        });
        it('should import VueApolloComposable from VueApolloComposableImportFrom config option', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { vueApolloComposableImportFrom: 'vue-apollo-composition-functions' }, {
                outputFile: 'graphql.ts',
            }));
...
\`;`);
    ...
    describe('Composition functions', () => {
      ...
      `);
            ...
            expect(content.content).toBeSimilarStringTo(`export function useFeedQuery(options: VueApolloComposable.UseQueryOptions<FeedQuery, FeedQueryVariables> | VueCompositionApi.Ref<VueApolloComposable.UseQueryOptions<FeedQuery, FeedQueryVariables>> | ReactiveFunction<VueApolloComposable.UseQueryOptions<FeedQuery, FeedQueryVariables>> = {}) {
             return VueApolloComposable.useQuery<FeedQuery, FeedQueryVariables>(FeedDocument, {}, options);
           }`);
            expect(content.content).toBeSimilarStringTo(`export function useSubmitRepositoryMutation(options: VueApolloComposable.UseMutationOptions<SubmitRepositoryMutation, SubmitRepositoryMutationVariables> | ReactiveFunction<VueApolloComposable.UseMutationOptions<SubmitRepositoryMutation, SubmitRepositoryMutationVariables>> = {}) {
      ...
      `);
            expect((await plugin(schema, [{ location: 'test-file.ts', document: documentWithHardcodedQuerySuffix }], {}, { outputFile: '' })).content).toContain('type NotificationsQueryQueryCompositionFunctionResult');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: documentWithHardcodedQuerySuffix }], { omitOperationSuffix: false }, { outputFile: '' })).content).toContain('type NotificationsQueryQueryCompositionFunctionResult');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: documentWithHardcodedQuerySuffix }], { omitOperationSuffix: true }, { outputFile: '' })).content).toContain('type NotificationsQueryCompositionFunctionResult');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: documentNoQuerySuffix }], { omitOperationSuffix: true }, { outputFile: '' })).content).toContain('type NotificationsCompositionFunctionResult');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: documentNoQuerySuffix }], { omitOperationSuffix: false }, { outputFile: '' })).content).toContain('type NotificationsQueryCompositionFunctionResult');
        });
      ...
      `);
            expect((await plugin(schema, [{ location: 'test-file.ts', document: documentWithQuerySuffix }], { omitOperationSuffix: true, dedupeOperationSuffix: true }, { outputFile: '' })).content).toContain('type NotificationsQueryCompositionFunctionResult');
        });
        it('Should not generate composition functions for query and mutation', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { withCompositionFunctions: false }, {
                outputFile: 'graphql.ts',
            }));
 ...
 */`;
      ...
      `);
            ...
            const mutationDocBlock = extract(content.content.substr(content.content.indexOf('/**', content.content.indexOf('/**') + 1)));
      ...
      `);
            const docs = [{ location: '', document: documents }];
            const content = (await plugin(schema, docs, { addDocBlocks: false }, {
                outputFile: 'graphql.ts',
            }));
            const queryDocBlock = extract(content.content.substr(content.content.indexOf('/**')));
...
//# sourceMappingURL=vue-apollo.spec.js.map