...
import { parse, buildClientSchema, buildASTSchema, buildSchema } from 'graphql';
...
describe('React Apollo', () => {
  ...
  `);
    const validateTypeScript = async (output, testSchema, documents, config) => {
    ...
    describe('Apollo Client v2 Backward Compatible', () => {
      ...
      `);
            ...
            const content = (await plugin(schema, docs, { reactApolloVersion: 2, withComponent: true, withHOC: true }, {
    ...
    describe('Issues', () => {
        ...
        ])('Issue #4887 - baseOptions of generated hooks should not be optional when there are required variables', async ({ document, optionalBaseOptions }) => {
            const docs = [
                {
                    location: '',
      ...
      \${FeedFragmentDoc}\`;`);
        ...
        it('Issue #2742 - Incorrect import prefix', async () => {
          ...
          `),
            ...
            expect(output).toContain(`export type Get_SomethingQueryResult = Apollo.QueryResult<Types.Get_SomethingQuery, Types.Get_SomethingQueryVariables>;`);
        });
        it('Issue #2826 - Incorrect prefix', async () => {
          ...
          `),
            ...
            const output = await validateTypeScript(content, schema, docs, config);
            expect(mergeOutputs([output])).toMatchSnapshot();
            expect(output).toContain(`export type Get_SomethingQueryResult = Apollo.QueryResult<GQLGet_SomethingQuery, GQLGet_SomethingQueryVariables>;`);
        });
    ...
    describe('Imports', () => {
        it('should import React and ReactApollo dependencies', async () => {
            ...
            const content = (await plugin(schema, docs, { withComponent: true }, {
                outputFile: 'graphql.tsx',
            }));
        ...
        it(`should use gql import from gqlImport config option`, async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { gqlImport: 'graphql.macro#gql' }, {
                outputFile: 'graphql.tsx',
            }));
      ...
      `);
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], {}, { outputFile: '' })).content).toContain('Apollo.QueryResult<NotificationsQueryQuery, NotificationsQueryQueryVariables>;');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: false }, { outputFile: '' })).content).toContain('Apollo.QueryResult<NotificationsQueryQuery, NotificationsQueryQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: true }, { outputFile: '' })).content).toContain('Apollo.QueryResult<NotificationsQuery, NotificationsQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: true }, { outputFile: '' })).content).toContain('Apollo.QueryResult<NotificationsQuery, NotificationsQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: false }, { outputFile: '' })).content).toContain('Apollo.QueryResult<NotificationsQuery, NotificationsQueryVariables>');
        });
      ...
      `);
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], {}, { outputFile: '' })).content).toContain('Apollo.QueryResult<NotificationsQueryQuery, NotificationsQueryQueryVariables>;');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { omitOperationSuffix: false }, { outputFile: '' })).content).toContain('Apollo.QueryResult<NotificationsQueryQuery, NotificationsQueryQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { omitOperationSuffix: true }, { outputFile: '' })).content).toContain('Apollo.QueryResult<NotificationsQuery, NotificationsQueryVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { omitOperationSuffix: true }, { outputFile: '' })).content).toContain('Apollo.QueryResult<Notifications, NotificationsVariables>');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { omitOperationSuffix: false }, { outputFile: '' })).content).toContain('Apollo.QueryResult<NotificationsQuery, NotificationsQueryVariables>');
        });
        ...
        it('should import ApolloReactHooks from apolloReactHooksImportFrom config option', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { apolloReactHooksImportFrom: 'react-apollo-hooks' }, {
                outputFile: 'graphql.tsx',
            }));
        ...
        it('should import ApolloReactCommon from apolloReactCommonImportFrom config option', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { apolloReactCommonImportFrom: 'custom-apollo-react-common' }, {
                outputFile: 'graphql.tsx',
            }));
...
\`;`);
    ...
    describe('Component', () => {
        ...
        it('should generate Document variable with noGraphQlTag', async () => {
            ...
            expect(content.content).toBeSimilarStringTo(`[{"kind":"Field","name":{"kind":"Name","value":"avatar_url"}}]}}]}}]}}]}}]} as unknown as DocumentNode;`);
      ...
      `);
        ...
        it('should generate a component with a custom suffix when specified', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { componentSuffix: 'Element', withComponent: true }, {
                outputFile: 'graphql.tsx',
            }));
            expect(content.content).toBeSimilarStringTo(`
      ...
      export const TestComponent = (props: TestComponentProps) => (
        ...
        it('should not add typesPrefix to Component', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { typesPrefix: 'I' }, {
                outputFile: 'graphql.tsx',
            }));
    ...
    describe('HOC', () => {
        it('should generate HOCs correctly', async () => {
            ...
            expect(content.content).toBeSimilarStringTo(`export type TestProps<TChildProps = {}, TDataName extends string = 'data'> = {
      ...
      TestProps<TChildProps, TDataName>>) {
        ...
        it('should not add typesPrefix to HOCs', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { typesPrefix: 'I', withHOC: true }, {
                outputFile: 'graphql.tsx',
            }));
        ...
        it('should generate mutation function signature correctly', async () => {
          ...
          `),
            ...
            const content = (await plugin(schema, docs, { withMutationFn: true }, {
                outputFile: 'graphql.tsx',
            }));
            expect(content.content).toContain(`export type SubmitCommentMutationFn = Apollo.MutationFunction<SubmitCommentMutation, SubmitCommentMutationVariables>;`);
            await validateTypeScript(content, schema, docs, {});
        });
    });
    describe('Hooks', () => {
  ...
  return Apollo.useMutation<SubmitRepositoryMutation, SubmitRepositoryMutationVariables>(SubmitRepositoryDocument, options);
      ...
      return Apollo.useMutation<SubmitRepositoryMutation, SubmitRepositoryMutationVariables>(SubmitRepositoryMutationDocument, options);
        ...
        it('Should not generate hooks for query and mutation', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { withHooks: false }, {
                outputFile: 'graphql.tsx',
            }));
...
export function useListenToCommentsSubscription(baseOptions?: Apollo.SubscriptionHookOptions<ListenToCommentsSubscription, ListenToCommentsSubscriptionVariables>) {
  ...
  return Apollo.useSubscription<ListenToCommentsSubscription, ListenToCommentsSubscriptionVariables>(ListenToCommentsDocument, options);
        ...
        it('Should not add typesPrefix to hooks', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { withHooks: true, typesPrefix: 'I' }, {
                outputFile: 'graphql.tsx',
            }));
      ...
      `);
            const docs = [{ location: '', document: documents }];
            const content = (await plugin(schema, docs, { withHooks: true, withComponent: false, withHOC: false }, {
                outputFile: 'graphql.tsx',
            }));
            expect(content.content).toBeSimilarStringTo(`
 ...
 */`;
      ...
      `);
            const docs = [{ location: '', document: documents }];
            const content = (await plugin(schema, docs, { withHooks: true, withComponent: false, withHOC: false, addDocBlocks: false }, {
                outputFile: 'graphql.tsx',
            }));
            const queryDocBlock = extract(content.content.substr(content.content.indexOf('/**')));
    ...
    `);
        it('should generate ResultType for Query if withResultType is true', async () => {
            ...
            expect(content.prepend).toContain(`import * as Apollo from '@apollo/client';`);
            expect(content.content).toContain(`export type TestQueryResult = Apollo.QueryResult<TestQuery, TestQueryVariables>;`);
            await validateTypeScript(content, schema, docs, {});
        });
        it('should NOT generate ResultType for Query if withResultType is false', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { ...config, withResultType: false }, {
                outputFile: 'graphql.tsx',
            }));
            expect(content.prepend).not.toContain(`import * as ApolloReactCommon from '@apollo/react-common';`);
            expect(content.content).not.toContain(`export type TestQueryResult = Apollo.QueryResult<TestQuery, TestQueryVariables>;`);
            await validateTypeScript(content, schema, docs, {});
        });
        ...
        it('should generate ResultType for Subscription if withResultType is true', async () => {
            ...
            expect(content.prepend).toContain(`import * as Apollo from '@apollo/client';`);
            expect(content.content).toContain(`export type TestSubscriptionResult = Apollo.SubscriptionResult<TestSubscription>;`);
            await validateTypeScript(content, schema, docs, {});
        });
        it('should NOT generate ResultType for Subscription if withResultType is false', async () => {
            ...
            expect(content.prepend).not.toContain(`import * as Apollo from '@apollo/react-common';`);
            expect(content.content).not.toContain(`export type TestSubscriptionResult = Apollo.SubscriptionResult<TestSubscription>;`);
            await validateTypeScript(content, schema, docs, {});
        });
    ...
    describe('MutationOptions', () => {
        ...
        it('should generate MutationOptions for Mutation if withMutationOptionsType is true', async () => {
            ...
            expect(content.prepend).toContain(`import * as Apollo from '@apollo/client';`);
            expect(content.content).toContain(`export type TestMutationOptions = Apollo.BaseMutationOptions<TestMutation, TestMutationVariables>;`);
            await validateTypeScript(content, schema, docs, {});
        });
        it('should NOT generate MutationOptions for Mutation if withMutationOptionsType is false', async () => {
            const docs = [{ location: '', document: mutationDoc }];
            const content = (await plugin(schema, docs, { ...config, withMutationOptionsType: false }, {
                outputFile: 'graphql.tsx',
            }));
            expect(content.prepend).not.toContain(`import * as ApolloReactCommon from '@apollo/react-common';`);
            expect(content.content).not.toContain(`export type TestMutationOptions = Apollo.BaseMutationOptions<TestMutation, TestMutationVariables>;`);
            await validateTypeScript(content, schema, docs, {});
        });
    ...
    describe('withRefetchFn', () => {
        it('should generate a function for use with refetchQueries', async () => {
            ...
            expect(content.content).toContain(`export function refetchTestQuery(variables?: TestQueryVariables) {
    ...
    `);
      ...
      `);
            ...
            const content = (await plugin(testSchema, docs, {
                withComponent: false,
                withHOC: false,
                withHooks: false,
                documentMode: DocumentMode.documentNode,
...
//# sourceMappingURL=react-apollo.spec.js.map