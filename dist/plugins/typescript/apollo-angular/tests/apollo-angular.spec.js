...
import { parse, buildClientSchema, buildSchema, extendSchema } from 'graphql';
  ...
  `);
    ...
    describe('Imports', () => {
      ...
      `);
            ...
            const content = (await plugin(modifiedSchema, docs, {}, {
                outputFile: 'graphql.ts',
            }));
      ...
      `);
        ...
        it('should output warning if documentMode = external and importDocumentNodeExternallyFrom is not set', async () => {
            ...
            const docs = [{ location: '', document: basicDoc }];
            await plugin(schema, docs, {
                documentMode: DocumentMode.external,
            }, {
                outputFile: 'graphql.ts',
            });
            // eslint-disable-next-line no-console
            expect(console.warn).toHaveBeenCalledWith('importDocumentNodeExternallyFrom must be provided if documentMode=external');
        });
        it('output warning if importOperationTypesFrom is set to something other than "Operations"', async () => {
            ...
            // eslint-disable-next-line no-console
            expect(console.warn).toHaveBeenCalledWith('importOperationTypesFrom only works correctly when left empty or set to "Operations"');
        });
        it('output warning if importOperationTypesFrom is set and documentMode is not "external"', async () => {
            ...
            // eslint-disable-next-line no-console
            expect(console.warn).toHaveBeenCalledWith('"importOperationTypesFrom" should be used with "documentMode=external" and "importDocumentNodeExternallyFrom"');
        });
    ...
    describe('Component', () => {
      ...
      `;
            ...
            const content = (await plugin(rootSchema, docs, {}, {
                outputFile: 'graphql.ts',
            }));
            expect(content.content).toBeSimilarStringTo(`
        @Injectable({
    ...
    describe('SDK Service', () => {
      ...
      `);
            const docs = [{ location: '', document: myFeed }];
            const content = (await plugin(modifiedSchema, docs, { sdkClass: true }, {
                outputFile: 'graphql.ts',
            }));
            // NgModule
...
//# sourceMappingURL=apollo-angular.spec.js.map