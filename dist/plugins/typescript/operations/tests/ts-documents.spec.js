...
describe('TypeScript Operations Plugin', () => {
  ...
  `);
    const validate = async (content, config = {}, pluginSchema = schema, usage = '', suspenseErrors = []) => {
    ...
    describe('Config', () => {
      ...
      `);
            ...
            expect(content).toBeSimilarStringTo(`export type TestQuery = { __typename?: 'Query', f?: Types.Maybe<Types.E>, user: { __typename?: 'User', id: string, f?: Types.Maybe<Types.E>, j?: Types.Maybe<any> } };`);
            await validate(content, config, schema, '', [`Cannot find namespace 'Types'.`]);
        });
    ...
    describe('Custom Operation Result Name Suffix', () => {
      ...
      `);
            ...
            expect(content).toBeSimilarStringTo(`export type NotificationsQueryVariables = Exact<{ [key: string]: never; }>;`);
            expect(content).toBeSimilarStringTo(`
    ...
    describe('Naming Convention & Types Prefix', () => {
      ...
      `);
            ...
            expect(content).toBeSimilarStringTo(`export type inotificationsqueryvariables = Exact<{ [key: string]: never; }>;`);
            expect(content).toBeSimilarStringTo(`
      ...
      `);
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], {}, { outputFile: '' })).content).toContain('export type NotificationsQueryQuery =');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], {}, { outputFile: '' })).content).toContain('export type MyFragmentFragment =');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: false }, { outputFile: '' })).content).toContain('export type NotificationsQueryQuery =');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: true }, { outputFile: '' })).content).toContain('export type NotificationsQuery =');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { dedupeOperationSuffix: true }, { outputFile: '' })).content).toContain('export type MyFragment =');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: true }, { outputFile: '' })).content).toContain('export type NotificationsQuery =');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: false }, { outputFile: '' })).content).toContain('export type NotificationsQuery =');
            expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { dedupeOperationSuffix: false }, { outputFile: '' })).content).toContain('export type MyFragment =');
            const withUsage = (await plugin(schema, [{ location: 'test-file.ts', document: ast3 }], { dedupeOperationSuffix: true, preResolveTypes: false }, { outputFile: '' })).content;
    ...
    `);
        expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { preResolveTypes: false }, { outputFile: '' })).content).toContain('export type NotificationsQueryQuery =');
        expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { preResolveTypes: false }, { outputFile: '' })).content).toContain('export type MyFragmentFragment =');
        expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { omitOperationSuffix: true, preResolveTypes: false }, { outputFile: '' })).content).toContain('export type NotificationsQuery =');
        expect((await plugin(schema, [{ location: 'test-file.ts', document: ast }], { omitOperationSuffix: true, preResolveTypes: false }, { outputFile: '' })).content).toContain('export type MyFragment =');
        expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { omitOperationSuffix: true, preResolveTypes: false }, { outputFile: '' })).content).toContain('export type Notifications =');
        expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { omitOperationSuffix: true, preResolveTypes: false }, { outputFile: '' })).content).toContain('export type My =');
        expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { omitOperationSuffix: false, preResolveTypes: false }, { outputFile: '' })).content).toContain('export type NotificationsQuery =');
        expect((await plugin(schema, [{ location: 'test-file.ts', document: ast2 }], { omitOperationSuffix: false, preResolveTypes: false }, { outputFile: '' })).content).toContain('export type MyFragment =');
        const withUsage = (await plugin(schema, [{ location: 'test-file.ts', document: ast3 }], { omitOperationSuffix: true, preResolveTypes: false }, { outputFile: '' })).content;
        expect(withUsage).toBeSimilarStringTo(`
      export type My = (
        { __typename?: 'Query' }
        & { notifications: Array<(
    ...
    describe('__typename', () => {
      ...
      `);
            ...
            expect(content).toBeSimilarStringTo(`export type Q1Query = (
          { __typename: 'Query' }
          & { test?: Maybe<(
            { __typename: 'Test' }
            & Pick<Test, 'foo'>
      ...
      `);
            ...
            expect(content).toContain(`export type Q1Query = { search: Array<{ __typename: 'Movie', id: string, title: string } | { __typename: 'Person', id: string, name: string }> };`);
            expect(content).toContain(`export type Q2Query = { search: Array<{ __typename: 'Movie', id: string, title: string } | { __typename: 'Person', id: string, name: string }> };`);
            await validate(content, config, testSchema);
        });
    ...
    describe('Selection Set', () => {
      ...
      `);
            const config = { preResolveTypes: false };
            try {
                await plugin(buildSchema(/* GraphQL */ `
            type Query {
              foo: String
            }
      ...
      `);
            ...
            await validate(content, config, testSchema, `function test(q: AaaQuery) {
      ...
      `);
            ...
            expect(content).toBeSimilarStringTo(`export type MeQueryVariables = Exact<{
    ...
    describe('Operation Definition', () => {
      ...
      `);
            ...
            expect(content).toBeSimilarStringTo(`export type TestQueryQueryVariables = Exact<{
      ...
      `);
            const { content } = await plugin(testSchema, [{ location: '', document: query }], { preResolveTypes: false }, {
      ...
      `);
            const coreContent = await tsPlugin(testSchema, [{ location: '', document: query }], {}, {
                outputFile: 'graphql.ts',
            });
            const pluginContent = await plugin(testSchema, [{ location: '', document: query }], {}, {
                outputFile: 'graphql.ts',
            });
      ...
      `);
            const { content } = await plugin(testSchema, [{ location: '', document: query }], { typesPrefix: 'PREFIX_', preResolveTypes: false }, {
                outputFile: 'graphql.ts',
            });
            expect(content).toBeSimilarStringTo(`
      ...
      `);
            const { content } = await plugin(testSchema, [{ location: '', document: query }], {}, {
                outputFile: 'graphql.ts',
            });
            expect(content).toBeSimilarStringTo(`
    ...
    describe('Union & Interfaces', () => {
      ...
      `);
            const { content } = await plugin(schema, [{ location: '', document: query }], { preResolveTypes: false }, {
                outputFile: 'graphql.ts',
            });
            expect(content).toBeSimilarStringTo(`
      ...
      `);
            ...
            const output = await validate(content, {}, testSchema, `
    ...
    describe('Issues', () => {
      ...
      `);
            ...
            expect(content).toMatchSnapshot();
            const result = await validate(content, {}, testSchema, `function test(q: QQuery) {
        if (q.hotel) {
...
function test(q: GetEntityBrandDataQuery): void {
  ...
  if (q.node.__typename === 'Company') {
    ...
    console.log('Movie:', q.node.id);
      ...
      `);
            const { content } = await plugin(schema, [{ location: '', document: fragment }], { preResolveTypes: false }, {
                outputFile: 'graphql.ts',
            });
            expect(content).toBeSimilarStringTo(`
      export type UserQuery = (
      ...
      `);
            const { content } = await plugin(schema, [{ location: '', document: query }], {
                skipTypename: true,
            }, {
                outputFile: 'graphql.ts',
            });
...
`);
      ...
      `);
            const { content } = await plugin(schema, [{ location: '', document }], { preResolveTypes: false }, {
                outputFile: 'graphql.ts',
            });
            expect(content).toBeSimilarStringTo(`
    ...
    describe('conditional directives handling', () => {
      ...
      `);
            const { content } = await plugin(schema, [{ location: '', document: fragment }], {
                preResolveTypes: true,
            }, {
                outputFile: 'graphql.ts',
            });
    ...
    `);
        const result = await plugin(schema, [{ location: 'test-file.ts', document: ast }], { preResolveTypes: false }, { outputFile: '' });
...
//# sourceMappingURL=ts-documents.spec.js.map