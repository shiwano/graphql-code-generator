  ...
  `);
    ...
    describe('Imports', () => {
        it('should import operation function dependencies', async () => {
            ...
            expect(content.prepend).toContain(`import { createMutationFunction, createSmartQueryOptionsFunction, createSmartSubscriptionOptionsFunction } from 'vue-apollo-smart-ops';`);
        ...
        it('should import operation function dependencies from configured packages', async () => {
            ...
            expect(content.prepend).toContain(`import { createMutationFunction, createSmartQueryOptionsFunction, createSmartSubscriptionOptionsFunction } from 'custom-operation-functions-package';`);
            await validateTypeScript(content, schema, docs, {});
        });
...
\`;`);
    ...
    describe('Operation functions', () => {
      ...
      `);
            ...
            expect(content.content).toBeSimilarStringTo(`export const useFeedQuery = createSmartQueryOptionsFunction<
  FeedQuery,
  FeedQueryVariables,
  ApolloError
>(FeedDocument);`);
            expect(content.content).toBeSimilarStringTo(`export const submitRepositoryMutation = createMutationFunction<
...
>(SubmitRepositoryMutationDocument);`);
        ...
        it('Should not generate operation functions for query and mutation', async () => {
            const docs = [{ location: '', document: basicDoc }];
            const content = (await plugin(schema, docs, { withSmartOperationFunctions: false }, {
                outputFile: 'graphql.ts',
            }));
      ...
      `);
            ...
            expect(content.content).toBeSimilarStringTo(`export const useListenToCommentsSubscription = createSmartSubscriptionOptionsFunction<
...
>(FeedDocument);`);
            ...
            expect(content.content).toBeSimilarStringTo(`export const useTestSubscription = createSmartSubscriptionOptionsFunction<
  TestSubscription,
...
//# sourceMappingURL=vue-apollo-smart-ops.spec.js.map