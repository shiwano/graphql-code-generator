...
describe('TypeScript Resolvers Plugin', () => {
    describe('Backward Compatability', () => {
      ...
      `);
            ...
            const resolversContent = (await plugin(testSchema, [], {
                contextType: 'Context',
    ...
    describe('Enums', () => {
      ...
      `);
            ...
            const mergedOutput = await validate(result, config, testSchema, `
      ...
      `);
            ...
            expect(mergedOutput).toContain(`export type MyEnumResolvers = EnumResolverSignature<{ A?: any, B?: any, C?: any }, ResolversTypes['MyEnum']>;`);
        });
    ...
    `);
        const config = { noSchemaStitching: true };
        const result = await plugin(testSchema, [], config, { outputFile: '' });
    ...
    it('Should allow to generate optional __resolveType', async () => {
        const result = (await plugin(schema, [], { optionalResolveType: true }, { outputFile: '' }));
        expect(result.content).toBeSimilarStringTo(`
...
export type ResolverFn<TResult, TParent, TContext, TArgs> = (
  ...
  info: GraphQLResolveInfo
    ...
    it('Should generate the correct resolver args type names when typesPrefix is specified', async () => {
        ...
        expect(o).toContain(`f?: Resolver<Maybe<TResolversTypes['String']>, ParentType, ContextType, RequireFields<TMyTypeFArgs, never>>;`);
    });
    ...
    `);
        const content = (await plugin(testSchema, [], { scalars: { Date: 'Date' } }, {
            outputFile: 'graphql.ts',
        }));
        expect(content.content).toBeSimilarStringTo(`
    ...
    `);
        ...
        const validateResolvers = (code) => {
            validateTs(mergeOutputs([
                tsContent,
                resolversContent,
                `
            import { PubSub } from 'graphql-subscriptions';
    ...
    describe('issues', () => {
      ...
      `);
            ...
            expect(o).not.toContain(`user?: Resolver<Maybe<ResolversTypesQL['User']>, ParentType, ContextType, RequireFields<QueryQLuserArgs, '_id'>>;`);
            expect(o).toContain(`user?: Resolver<Maybe<ResolversTypesQL['User']>, ParentType, ContextType, RequireFields<QueryUserArgsQL, '_id'>>;`);
            expect(o).toContain(`me?: Resolver<Maybe<ResolversTypesQL['User']>, ParentType, ContextType>;`);
            expect(o).toContain(`user2?: Resolver<Maybe<ResolversTypesQL['User']>, ParentType, ContextType, RequireFields<QueryUser2ArgsQL, never>>;`);
        });
    ...
    it('Should generate resolvers with replaced internalResolversPrefix if specified', async () => {
        const result = (await plugin(schema, [], { internalResolversPrefix: '' }, { outputFile: '' }));
...
//# sourceMappingURL=ts-resolvers.spec.js.map