...
describe('TypeScript', () => {
    ...
    describe('description to comment', () => {
      ...
      `);
            const result = (await plugin(schema, [], { enumsAsConst: true }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
      export const MyEnum = {
        ABC: 'A_B_C',
        XYZ: 'X_Y_Z',
    ...
    describe('disable comment generation', () => {
      ...
      `);
            const result = (await plugin(schema, [], { enumsAsTypes: true, disableDescriptions: true }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
      export type MyEnum =
        | 'A' 
        | 'B';`);
      ...
      `);
            const result = (await plugin(schema, [], { enumsAsTypes: true, disableDescriptions: false }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
      /** custom enum */
      export type MyEnum =
        /** this is a */
    ...
    describe('Issues', () => {
      ...
      type PullRequestReview {
          threads(first: Int!, last: Int!): Int
            ...
            const result = (await plugin(schema, [], { addUnderscoreToArgsType: true }, { outputFile: '' }));
    ...
    describe('Config', () => {
      ...
      `);
            const result = (await plugin(schema, [], { avoidOptionals: { inputValue: true } }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
        export type MyInput = {
          foo: Maybe<Scalars['String']>;
          bar: Scalars['String'];
    ...
    `);
      ...
      enum MyEnum {
        ...
        B
            ...
            const result = (await plugin(schema, [], { enumsAsTypes: true, futureProofEnums: true }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
      export type MyEnum = 
        | 'A' 
        | 'B' 
    ...
    `);
      ...
      enum MyEnum {
        ...
        B
            ...
            const result = (await plugin(schema, [], { futureProofEnums: true }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
      export enum MyEnum {
        A = 'A',
        B = 'B'
    ...
    `);
      ...
      `);
            const result = (await plugin(testSchema, [{ location: '', document: query }], {}, {
    ...
    describe('Scalars', () => {
      ...
      type MyType {
        ...
        bar: MyScalar!
            ...
            const result = (await plugin(schema, [], { defaultScalarType: 'unknown' }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
      export type Scalars = {
        ID: string;
        String: string;
      ...
      scalar A
      `);
            const result = (await plugin(schema, [], { wrapFieldDefinitions: true }, { outputFile: '' }));
      ...
      scalar A
      `);
            const result = (await plugin(schema, [], { fieldWrapperValue: 'T | Promise<T>', wrapFieldDefinitions: true }, { outputFile: '' }));
    ...
    describe('Object (type)', () => {
      ...
      `);
            const result = (await plugin(schema, [], { wrapEntireFieldDefinitions: true }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
        export type ListOfStrings = {
          __typename?: 'ListOfStrings';
      ...
      `);
            const result = (await plugin(schema, [], { wrapEntireFieldDefinitions: true, wrapFieldDefinitions: true }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
        export type ListOfStrings = {
          __typename?: 'ListOfStrings';
    ...
    describe('Naming Convention & Types Prefix', () => {
        it('Should use custom namingConvention for type name and args typename', async () => {
            ...
            const result = (await plugin(schema, [], { namingConvention: 'change-case-all#lowerCase' }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
        export type mytypefooargs = {
          a: Scalars['String'];
          b?: Maybe<Scalars['String']>;
    ...
    `);
        ...
        it('Should use custom namingConvention and add custom prefix', async () => {
            const schema = buildSchema(`type MyType { foo(a: String!, b: String, c: [String], d: [Int!]!): String }`);
            const result = (await plugin(schema, [], { namingConvention: 'change-case-all#lowerCase', typesPrefix: 'I' }, { outputFile: '' }));
            expect(result.content).toBeSimilarStringTo(`
    ...
    `);
      ...
      `);
        ...
        it('Should generate correctly types for inputs with default value - #4273', async () => {
            const schema = buildSchema(`input MyInput { a: String = "default", b: String! = "default", c: String, d: String! }`);
            const result = await plugin(schema, [], {}, { outputFile: '' });
            expect(result.content).toBeSimilarStringTo(`
        export type MyInput = {
          a?: Maybe<Scalars['String']>;
    ...
    describe('Enum', () => {
      ...
      `);
        ...
        it('Should build enum correctly with custom imported enum from namspace with different name', async () => {
            const schema = buildSchema(`enum MyEnum { A, B, C }`);
            const result = (await plugin(schema, [], { enumValues: { MyEnum: './my-file#NS.ETest' } }, { outputFile: '' }));
            expect(result.content).not.toContain(`export enum MyEnum`);
        ...
        it('Should import all enums from a single file when specified as string', async () => {
            ...
            const result = (await plugin(schema, [], { enumValues: './my-file' }, { outputFile: '' }));
            expect(result.content).not.toContain(`export enum MyEnum`);
        ...
        it('Should re-export external enums', async () => {
            const schema = buildSchema(`enum MyEnum { A, B, C } enum MyEnum2 { X, Y, Z }`);
            const result = (await plugin(schema, [], { enumValues: { MyEnum: './my-file#MyEnum', MyEnum2: './my-file#MyEnum2X' } }, { outputFile: '' }));
            expect(result.content).toContain(`export { MyEnum };`);
      ...
      `);
            const result = (await plugin(schema, [], { allowEnumStringTypes: true }, { outputFile: '' }));
            validateTs(result);
