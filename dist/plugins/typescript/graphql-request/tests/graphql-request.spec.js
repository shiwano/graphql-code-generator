...
describe('graphql-request', () => {
  ...
  `);
    const validate = async (content, config, docs, pluginSchema, usage) => {
  ...
  if (result.feed) {
    if (result.feed[0]) {
      const id = result.feed[0].id
            ...
            expect(result.content).toContain(`(FeedDocument, variables, {...requestHeaders, ...wrappedRequestHeaders}), 'feed');`);
    ...
    describe('issues', () => {
      ...
      `);
        ...
        it('#4748 - integration with importDocumentNodeExternallyFrom', async () => {
            ...
            expect(output).toContain(`(Operations.FeedDocument, variables, {...requestHeaders, ...wrappedRequestHeaders}), 'feed');`);
            expect(output).toContain(`(Operations.Feed2Document, variables, {...requestHeaders, ...wrappedRequestHeaders}), 'feed2');`);
            expect(output).toContain(`(Operations.Feed3Document, variables, {...requestHeaders, ...wrappedRequestHeaders}), 'feed3');`);
        });
    });
});
