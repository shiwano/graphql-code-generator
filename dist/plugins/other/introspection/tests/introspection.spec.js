...
describe('Introspection template', () => {
    ...
    `);
        ...
        const introspection = JSON.stringify(introspectionFromSchema(schema, { descriptions: true, schemaDescription: false, specifiedByUrl: false }), null, 2);
    ...
    `);
        ...
        const introspection = JSON.stringify(introspectionFromSchema(schema, { descriptions: true, schemaDescription: false, specifiedByUrl: false }));
        expect(introspection).toEqual(content);
    });
    ...
    `);
        ...
        const queryType = types.find(type => type.name === introspection.__schema.queryType.name);
        // scalar _Any
